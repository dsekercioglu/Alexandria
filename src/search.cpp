...
static bool IsRepetition(const S_Board *pos, const bool pvNode) {
  ...
  int counter = 1;
  // How many moves back should we look at most, aka our distance to the last
  // irreversible move
  ...
  // Scan backwards from the first position where a repetition is possible (4
  // half moves ago) for at most distance steps
...
// If we triggered any of the rules that forces a draw or we know the position
// is a draw return a draw score
bool IsDraw(S_Board *pos, const bool pvNode) {
  // if it's a 3-fold repetition, the fifty moves rule kicked in or there isn't
  // enough material on the board to give checkmate then it's a draw
  return IsRepetition(pos, pvNode) || Is50MrDraw(pos) || MaterialDraw(pos);
}

// ClearForSearch handles the cleaning of the post and the info parameters to
// start search from a clean state
...
bool SEE(const S_Board *pos, const int move, const int threshold) {
  ...
  while (true) {
    ...
    occupied ^=
        1ULL << (GetLsbIndex(myAttackers & pos->GetPieceColorBB(pt, side ^ 1)));
...
int GetBestMove(const PvTable *pvTable) { return pvTable->pvArray[0][0]; }

// Starts the search process, this is ideally the point where you can start a
// multithreaded search
void RootSearch(int depth, S_ThreadData *td, S_UciOptions *options) {
  // Init a thread_data object for each helper thread that doesn't have one
  // already
...
// SearchPosition is the actual function that handles the search, it sets up the
// variables needed for the search , calls the AspirationWindowSearch function
// and handles the console output
void SearchPosition(int startDepth, int finalDepth, S_ThreadData *td,
                    S_UciOptions *options) {
  // variable used to store the score of the best move found by the search
  // (while the move itself can be retrieved from the triangular PV table)
  int score = 0;
  ...
  for (int currentDepth = startDepth; currentDepth <= finalDepth;
       currentDepth++) {
    score = AspirationWindowSearch(averageScore, currentDepth, td);
    averageScore =
        averageScore == score_none ? score : (averageScore + score) / 2;
    ...
    if (td->id == 0) {
      ...
      // use the previous search to adjust some of the time management
      // parameters, do not scale movetime time controls
      if (td->RootDepth > 7 && td->info.timeset) {
        ScaleTm(td, bestMoveStabilityFactor);
      }

      // check if we just cleared a depth and more than OptTime passed, or we
      // used more than the give nodes
...
int AspirationWindowSearch(int prev_eval, int depth, S_ThreadData *td) {
  ...
  // We set an expected window for the score at the next search depth, this
  // window is not 100% accurate so we might need to try a bigger window and
  // re-search the position
  int delta = 12;
  // define initial alpha beta bounds
  int alpha = -MAXSCORE;
  int beta = MAXSCORE;

  // only set up the windows is the search depth is bigger or equal than
  // Aspiration_Depth to avoid using windows when the search isn't accurate
  // enough
  if (depth >= 3) {
  ...
  while (true) {
    ...
    // We fell outside the window, so try again with a bigger window, since we
    // failed low we can adjust beta to decrease the total window size
    if (score <= alpha) {
      beta = (alpha + beta) / 2;
...
int Negamax(int alpha, int beta, int depth, const bool cutNode,
            S_ThreadData *td, Search_stack *ss) {
  // Extract data structures from ThreadData
  S_Board *pos = &td->pos;
  ...
  if (!rootNode) {
    // If position is a draw return a randomized draw score to avoid 3-fold
    // blindness
  ...
  // Probe the TT for useful previous search informations, we avoid doing so if
  // we are searching a singular extension
  const bool ttHit =
      excludedMove ? false : ProbeHashEntry(pos->GetPoskey(), &tte);
  const int ttScore = ttHit ? ScoreFromTT(tte.score, ss->ply) : score_none;
  const int ttMove =
      ttHit ? MoveFromTT(tte.move, pos->PieceOn(From(tte.move))) : NOMOVE;
  const uint8_t ttFlag = ttHit ? tte.wasPv_flags & 3 : HFNONE;
  // If we found a value in the TT for this position, and the depth is equal or
  // greater we can return it (pv nodes are excluded)
  if (!pvNode && ttScore != score_none && tte.depth >= depth &&
      ((ttFlag == HFUPPER && ttScore <= alpha) ||
       (ttFlag == HFLOWER && ttScore >= beta) || ttFlag == HFEXACT))
    return ttScore;
  ...
  // If we are in check or searching a singular extension we avoid pruning
  ...
  if (ttHit) {
    // If the value in the TT is valid we use that, otherwise we call the static
    // evaluation function
    eval = ss->staticEval =
        tte.eval != score_none ? tte.eval : EvalPosition(pos);

    // We can also use the tt score as a more accurate form of eval
    if (ttScore != score_none &&
        ((ttFlag == HFUPPER && ttScore < eval) ||
         (ttFlag == HFLOWER && ttScore > eval) || ttFlag == HFEXACT))
      eval = ttScore;
  } else {
    ...
    if (!excludedMove)
      ...
      StoreHashEntry(pos->posKey, NOMOVE, score_none, eval, HFNONE, 0, pvNode,
                     ttPv);
  }

  // Improving is a very important modifier to many heuristics. It checks if our
  // static eval has improved since our last move. As we don't evaluate in
  // check, we look for the first ply we weren't in check between 2 and 4 plies
  // ago. If we find that static eval has improved, or that we were in check
  // both 2 and 4 plies ago, we set improving to true.
  ...
  if (!pvNode) {
    ...
    if (depth < 10 && abs(eval) < mate_found &&
        eval - 91 * (depth - improving) >= beta)
      return eval;

    // Null move pruning: If our position is so good that we can give the
    // opponent a free move and still fail high, return early. At higher depth
    // we do a reduced search with null move pruning disabled (ie verification
    // search) to prevent falling into zugzwangs.
    if (eval >= ss->staticEval && eval >= beta && ss->ply &&
        (ss - 1)->move != NOMOVE && depth >= 3 && ss->ply >= td->nmpPlies &&
      ...
      int nmpScore =
          -Negamax<false>(-beta, -beta + 1, depth - R, !cutNode, td, ss + 1);
      ...
      if (nmpScore >= beta) {
        ...
        // Verification search to avoid zugzwangs: if we are at an high enough
        // depth we perform another reduced search without nmp for at least
        // nmpPlies
        td->nmpPlies = ss->ply + (depth - R) * 2 / 3;
        int verificationScore =
            Negamax<false>(beta - 1, beta, depth - R, false, td, ss);
        td->nmpPlies = 0;
...
moves_loop:
  ...
  while ((move = NextMove(&mp, false)) != NOMOVE) {
    ...
    if (!rootNode && BoardHasNonPawns(pos, pos->side) &&
        bestScore > -mate_found) {

      if (!SkipQuiets) {

        // Movecount pruning: if we searched enough moves and we are not in
        // check we skip the rest
        if (!pvNode && !inCheck && totalMoves > lmp_margin[depth][improving]) {
          SkipQuiets = true;
        }

        // lmrDepth is the current depth minus the reduction the move would
        // undergo in lmr, this is helpful because it helps us discriminate the
        // bad moves with more accuracy
        const int lmrDepth =
            std::max(0, depth - reductions[isQuiet][depth][totalMoves]);

        // Futility pruning: if the static eval is so low that even after adding
        // a bonus we are still under alpha we can stop trying quiet moves
        if (!inCheck && lmrDepth < 11 &&
            ss->staticEval + 250 + 150 * lmrDepth <= alpha) {
          SkipQuiets = true;
        }
      }

      // See pruning: prune all the moves that have a SEE score that is lower
      // than our threshold
      if (depth <= 8 && !SEE(pos, move, see_margin[depth][isQuiet]))
        continue;
    }
    ...
    if (ss->ply < td->RootDepth * 2) {
      // Search extension
      if (!rootNode && (depth >= 7 || !inCheck) && move == ttMove &&
          !excludedMove && (ttFlag & HFLOWER) && abs(ttScore) < mate_found &&
          tte.depth >= depth - 3) {
        const int singularBeta = ttScore - depth;
        const int singularDepth = (depth - 1) / 2;

        ss->excludedMove = ttMove;
        int singularScore = ss->staticEval;
        if (depth >= 7) {
          singularScore = Negamax<false>(singularBeta - 1, singularBeta,
                                         singularDepth, cutNode, td, ss);
        }
        ...
        if (singularScore < singularBeta) {
          ...
          if (!pvNode && singularScore < singularBeta - 17 &&
              ss->doubleExtensions <= 11) {
            extension =
                2 + (!isTactical(ttMove) && singularScore < singularBeta - 100);
            ss->doubleExtensions = (ss - 1)->doubleExtensions + 1;
          }
        } else if (singularBeta >= beta)
          return singularBeta;

        // If we didn't successfully extend and our TT score is above beta
        // reduce the search depth
        else if (ttScore >= beta)
          extension = -2;

        // If we are expecting a fail-high both based on search states from
        // previous plies and based on TT bound but our TT move is not singular
        // and our TT score is failing low, reduce the search depth
    ...
    // Conditions to consider LMR. Calculate how much we should reduce the
    // search depth.
    if (totalMoves > 1 + pvNode && depth >= 3 && (isQuiet || !ttPv)) {
      ...
      // Decrease the reduction for moves that have a good history score and
      // increase it for moves with a bad score
      depthReduction -= moveHistory / 16384;

      // adjust the reduction so that we can't drop into Qsearch and to prevent
      // extensions
      ...
      score =
          -Negamax<false>(-alpha - 1, -alpha, reducedDepth, true, td, ss + 1);

      // if we failed high on a reduced node we'll search with a reduced window
      // and full depth
      if (score > alpha && newDepth > reducedDepth) {
        // Based on the value returned by our reduced search see if we should
        // search deeper or shallower, this is an exact yoink of what SF does
        // and frankly i don't care lmao
        ...
        if (newDepth > reducedDepth)
          score = -Negamax<false>(-alpha - 1, -alpha, newDepth, !cutNode, td,
                                  ss + 1);

        int bonus =
            score > alpha ? history_bonus(depth) : -history_bonus(depth);
        updateCHScore(sd, ss, move, bonus);
      }
    }
    // If we skipped LMR and this isn't the first move of the node we'll search
    // with a reduced window and full depth
    else if (!pvNode || totalMoves > 1) {
      score =
          -Negamax<false>(-alpha - 1, -alpha, newDepth, !cutNode, td, ss + 1);
    }

    // PVS Search: Search the first move and every move that beat alpha with
    // full depth and a full window
    ...
    UnmakeMove(move, pos);
    if (td->id == 0 && rootNode)
      td->nodeSpentTable[From(move)][To(move)] +=
          info->nodes - nodesBeforeSearch;
    ...
    if (score > bestScore) {
      ...
      if (score > alpha) {
        ...
        if (pvNode) {
          ...
          for (int nextPly = ss->ply + 1;
               nextPly < pvTable->pvLength[ss->ply + 1]; nextPly++) {
            pvTable->pvArray[ss->ply][nextPly] =
                pvTable->pvArray[ss->ply + 1][nextPly];
          }
          pvTable->pvLength[ss->ply] = pvTable->pvLength[ss->ply + 1];
        }

        if (score >= beta) {
          // If the move that caused the beta cutoff is quiet we have a killer
          // move
          if (isQuiet) {
            // Don't update killer moves if it would result in having 2
            // identical killer moves
  ...
  // We don't have any legal moves to make in the current postion. If we are in
  // singular search, return -infinite. Otherwise, if the king is in check,
  // return a mate score, assuming closest distance to mating position. If we
  // are in neither of these 2 cases, it is stalemate.
  if (totalMoves == 0) {
    return excludedMove ? -MAXSCORE : inCheck ? -mate_score + ss->ply : 0;
  }
  // Set the TT flag based on whether the bestScore is better than beta and if
  // it's not based on if we changed alpha or not
  int flag = bestScore >= beta    ? HFLOWER
             : alpha != old_alpha ? HFEXACT
                                  : HFUPPER;

  if (!excludedMove)
    StoreHashEntry(pos->posKey, MoveToTT(bestMove),
                   ScoreToTT(bestScore, ss->ply), ss->staticEval, flag, depth,
                   pvNode, ttPv);
  // return best score
  return bestScore;
}
...
int Quiescence(int alpha, int beta, S_ThreadData *td, Search_stack *ss) {
  ...
  const int ttMove =
      ttHit ? MoveFromTT(tte.move, pos->PieceOn(From(tte.move))) : NOMOVE;
  const uint8_t ttFlag = ttHit ? tte.wasPv_flags & 3 : HFNONE;
  // If we found a value in the TT for this position, we can return it (pv nodes
  // are excluded)
  ...
  // If we have a ttHit with a valid eval use that
    ...
    if (ttScore != score_none &&
        ((ttFlag == HFUPPER && ttScore < bestScore) ||
         (ttFlag == HFLOWER && ttScore > bestScore) || ttFlag == HFEXACT))
      bestScore = ttScore;
  }
  ...
  Movepicker mp;
  // If we aren't in check we generate just the captures, otherwise we generate
  // all the moves
  ...
  while ((move = NextMove(&mp, bestScore > -mate_found)) != NOMOVE) {
    ...
    // Futility pruning. If static eval is far below alpha, only search moves
    // that win material.
    if (bestScore > -mate_found && !inCheck && !isPromo(move) &&
        !isEnpassant(move) && BoardHasNonPawns(pos, pos->side)) {
    ...
    if (score > bestScore) {
      ...
      if (score > alpha) {
        ...
        // if the score is better than or equal to beta break the loop because
        // we failed high
        if (score >= beta)
          break;
  ...
  // Set the TT flag based on whether the bestScore is better than beta, for
  // qsearch we never use the exact flag
  int flag = bestScore >= beta ? HFLOWER : HFUPPER;

  StoreHashEntry(pos->posKey, MoveToTT(bestmove), ScoreToTT(bestScore, ss->ply),
                 ss->staticEval, flag, 0, pvNode, ttPv);
