...
int Quiescence(int alpha, int beta, S_Board *pos, S_SearchINFO *info)
{
	...
	for (int count = 0; count < move_list->count; count++)
	{
		...
		if (moves_searched > 0 && BestScore > -ISMATE && get_move_capture(move) && move_Score < 600000000)
			continue;
	...
	int flag = BestScore >= beta ? HFBETA : (alpha != old_alpha) ? HFEXACT
																 : HFALPHA;
			...
			int DoNull)
{
	...
	// If we found a value in the TT we can return it
	if (pos->ply && !pv_node && ttHit && tte.depth >= depth && MoveExists(pos, tte.move))
	{
	...
	if (!pv_node && depth < rfp_depth && static_eval - futility(depth, improving) >= beta)
		return static_eval;
	...
	if (!pv_node && DoNull && static_eval >= beta && pos->ply && depth >= nmp_depth && BoardHasNonPawns(pos, pos->side))
	{
...
moves_loop:
	...
	for (int count = 0; count < move_list->count; count++)
	{
		...
		int history = pos->searchHistory[pos->pieces[get_move_source(move)]][get_move_target(move)];

		if (!pv_node && !in_check && depth <= 4 && isQuiet && history < 0)
		{
			continue;
		}
		...
		if (!root_node && depth >= 7 && move == tte.move && !excludedMove && (tte.flags & HFBETA) && abs(tte.score) < ISMATE && tte.depth >= depth - 3)
		{
	...
	if (move_list->count == 0)
	{
		...
		BestScore = excludedMove ? alpha : in_check ? (-mate_value + pos->ply)
													: 0;
	}
	// if we updated alpha we have an exact score, otherwise we only have an upper bound (for now the beta flag isn't actually ever used)
